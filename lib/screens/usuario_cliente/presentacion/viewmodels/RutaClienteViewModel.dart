...
import 'package:taxi_app/widgets/loader_widget.dart';
...
class Rutaclienteviewmodel extends ChangeNotifier {
    ...
    Future<void> cancelarSolicitud(String solicitudId) async {
      ...
      try {
        await FirebaseFirestore.instance
            ...
            .doc(solicitudId)
            .update({'estado': 'cancelado', 'cancelledAt': FieldValue.serverTimestamp()});
      } catch (e) {
  ...
  void escucharEstadoSolicitud(String solicitudId, BuildContext context) {
    ...
    _solicitudStateSub = FirebaseFirestore.instance
      ...
      // Solo reaccionar si el estado cambió a 'en camino' y no era 'en camino' antes
      if (ultimoEstado != estado && estado == 'en camino') {
        debugPrint('[RutaClienteViewModel] Cambio detectado a "en camino". Navegando a RutaClienteDestino...');
        ultimoEstado = estado;
        // Mostrar loader antes de navegar
        showDialog(
          context: context,
          barrierDismissible: false,
          builder: (_) => const LoaderPreparandoRuta(),
        );
        // Cargar la clase en segundo plano
        final rutaClienteDestinoWidget = ChangeNotifierProvider(
        ...
        Navigator.of(context).pushReplacement(
          MaterialPageRoute(
            builder: (context) => rutaClienteDestinoWidget,
          ),
        );
      } else {
        ultimoEstado = estado;
  ...
  void iniciarChat(String solicitudId) {
    ...
    _chatSub = chatService.listenMessages(solicitudId).listen(
      (ms) {
        // Mostrar notificación siempre que llegue un nuevo mensaje
        if (ms.length > mensajes.length) {
  ...
  Future<void> cargarDatosConductorYUbicacionCliente(String solicitudId) async {
    ...
    try {
      ...
      if (conductor is Map<String, dynamic>) {
        ...
        final foto = conductor['foto'] ?? conductor['photo'] ?? conductor['photoUrl'] ?? conductor['imagen'];
        ...
        final fotoV = conductor['fotoVehiculo'] ?? conductor['foto_carro'] ?? conductor['fotoAuto'] ?? '';
      ...
      if (cliente is Map<String, dynamic>) {
        if (cliente['ubicacion'] is Map<String, dynamic>) {
          ...
          lngCliente = ubicacion['lng'] != null
              ...
              : null;
          debugPrint('[DEBUG] Ubicación cliente: lat=$latCliente, lng=$lngCliente');
        } else {
          latCliente = null;
          lngCliente = null;
